...
class LoadDualStream:
    ...
    # ----------------------------------------------------------
    #  load_image — y hệt LoadImagesAndLabels.load_image()
    # ----------------------------------------------------------
    def _load_image(self, f):
        """cv2.imread + resize giữ tỉ lệ (cạnh dài = img_size)"""
        im = cv2.imread(f)
        assert im is not None, f'Không đọc được ảnh: {f}'
        h0, w0 = im.shape[:2]
        r = self.img_size / max(h0, w0)
        if r != 1:
            interp = cv2.INTER_LINEAR if r > 1 else cv2.INTER_AREA
            im = cv2.resize(im, (int(w0 * r), int(h0 * r)), interpolation=interp)
        return im, (h0, w0), im.shape[:2]   # img_resized, hw_orig, hw_resized
    ...
    def _preprocess(self, f):
        ...
        """
        img0_bgr = cv2.imread(f)                        # ảnh gốc, giữ để vẽ
        assert img0_bgr is not None, f'Không đọc được: {f}'

        # bước 1: resize giữ tỉ lệ (= load_image)
        img, (h0, w0), (h, w) = self._load_image(f)

        # bước 2: letterbox — pad về bội stride
        #   val.py dùng auto=False, scaleup=self.augment
        #   lúc val augment=False → scaleup=False
...
def run(
    ...
    source         = ROOT / 'datasets/rgb',   # file .txt/.xml hoặc thư mục RGB
    source2        = ROOT / 'datasets/mask',  # file .txt/.xml hoặc thư mục mask
    imgsz          = (640, 640),
    conf_thres     = 0.55,
    iou_thres      = 0.45,
    max_det        = 1000,
    ...
    # ══════════════════════════════════════════════════════════
    for (path, path2,
         img_rgb_np,  im0_rgb,  shapes_rgb,
         img_mask_np, im0_mask, shapes_mask,
         s) in dataset:
        ...
        # 1. numpy → tensor ───────────────────────────────────
        t1  = time_sync()
        im  = to_tensor(img_rgb_np,  device, half)
        im2 = to_tensor(img_mask_np, device, half)
        t2  = time_sync()
        dt[0] += t2 - t1

        # 2. Inference dual-stream (y hệt val.py) ─────────────
                ...
                if save_crop:
                    save_one_box(xyxy, im0.copy(),
                                 file=save_dir / 'crops' / names[c_int] /
                                      f'{Path(path).stem}_{dataset.index}.jpg',
                                 BGR=True)
        ...
        cur_fps    = 1.0 / frame_time if frame_time > 0 else 0.0
        fps_deque.append(cur_fps)
        max_fps = max(max_fps, cur_fps)
        avg_fps = sum(fps_deque) / len(fps_deque)
        cv2.putText(im0, f'FPS: {avg_fps:.1f}  Max: {max_fps:.1f}',
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

        # 6. Hiển thị — 1 cửa sổ duy nhất ────────────────────
        if view_img:
    ...
    LOGGER.info(f'FPS — Max: {max_fps:.1f}  Avg: {avg_fps:.1f}')
