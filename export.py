...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    try:
        ...
        d = {"shape": im[0].shape, "stride": int(max(model.stride)), "names": model.names}
...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        torch.onnx.export(model, im, f, verbose=False, opset_version=opset,
                          ...
                          output_names=['output'],
                          dynamic_axes={'images1': {0: 'batch', 2: 'height', 3: 'width'},  # shape(1,3,640,640)
                                        'images2': {0: 'batch', 2: 'height', 3: 'width'},
        ...
        if simplify:
            try:
                ...
                model_onnx, check = onnxsim.simplify(
                    ...
                    input_shapes={'images1': list(im[0].shape), 'images2': list(im[1].shape)} if dynamic else None)
...
def export_engine(model, im, file, train, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    try:
        ...
        config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, workspace << 30)  # fix TRT 8.4+ and TRT 10 compatibility
        ...
        with open(f, 'wb') as t:
            t.write(builder.build_serialized_network(network, config))
        LOGGER.info(f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        return f
    except Exception as e:
        LOGGER.info(f'\n{prefix} export failure: {e}')
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    im1 = torch.zeros(batch_size, 3, *imgsz).to(device)  # image size(1,3,320,192) BCHW iDetection
    im2 = torch.zeros(batch_size, 3, *imgsz).to(device)
    im = (im1, im2)

    # Update model
    if half:
        im1, im2 = im1.half(), im2.half()
        im = (im1, im2)
