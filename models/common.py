...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False):
        ...
        if pt:  # PyTorch
            ...
            num = model.num_io_tensors if is_trt10 else model.num_bindings
            for i in range(num):
                name = model.get_tensor_name(i) if is_trt10 else model.get_binding_name(i)
                dtype = trt.nptype(model.get_tensor_dtype(name) if is_trt10 else model.get_binding_dtype(i))
                shape = tuple(model.get_tensor_shape(name) if is_trt10 else model.get_binding_shape(i))
                data = torch.from_numpy(np.empty(shape, dtype=np.dtype(dtype))).to(device)
                bindings[name] = Binding(name, dtype, shape, data, int(data.data_ptr()))
                is_input = (model.get_tensor_mode(name) == trt.TensorIOMode.INPUT) if is_trt10 else model.binding_is_input(i)
                if is_input and dtype == np.float16:
                    fp16 = True
            binding_addrs = OrderedDict((n, d.ptr) for n, d in bindings.items())
            context = model.create_execution_context()
            batch_size = bindings['images1'].shape[0] if 'images1' in bindings else bindings['images'].shape[0]
        elif coreml:  # CoreML
        ...
        if self.pt or self.jit:  # PyTorch
            ...
            if 'images1' in self.bindings and 'images2' in self.bindings:
                if im.shape == self.bindings['images1'].shape:
                    self.binding_addrs['images1'] = int(im.data_ptr())
                    self.binding_addrs['images2'] = int(im2.data_ptr())
                else:
                    self.bindings['images1'].data.fill_(0)
                    self.bindings['images1'].data[:b] = im
                    self.bindings['images2'].data.fill_(0)
                    self.bindings['images2'].data[:b] = im2
                    self.binding_addrs['images1'] = int(self.bindings['images1'].ptr)
                    self.binding_addrs['images2'] = int(self.bindings['images2'].ptr)
            else:
                if im.shape == self.bindings['images'].shape:
                    self.binding_addrs['images'] = int(im.data_ptr())
                else:
                    self.bindings['images'].data.fill_(0)
                    self.bindings['images'].data[:b] = im
                    self.binding_addrs['images'] = int(self.bindings['images'].ptr)
            self.context.execute_v2(list(self.binding_addrs.values()))
            y = self.bindings['output'].data[:b]
...
# ==========================================
# YOLOv11 Modules (C3k2, C2PSA, etc.)
# ==========================================
...
class C3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        # YOLOv5 Bottleneck doesn't take k parameter by default. 
        # But we can override cv2 inside Bottleneck if needed, or just use the standard Bottleneck
        # To be fully compatible with YOLOv11 C3k, we use the standard YOLOv5 Bottleneck which has 3x3 kernel.
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
