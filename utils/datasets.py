...
class LoadImages:
    ...
    def __init__(self, path,path2, img_size=640, stride=32, auto=True):
        ...
        self.files2 = images2 + [path2] * nv  # Use path2 as mask source for videos
        # Clip to the shortest list so they stay in sync
        min_ni = min(len(images), len(images2))
        self.files = self.files[:min_ni] + (images + videos)[len(images):]
        self.files2 = self.files2[:min_ni] + (images2 + videos)[len(images2):]
        self.nf = min_ni + nv  # number of files
    ...
    def __next__(self): #输出下一项
        ...
        if self.video_flag[self.count]: #如果为视频
            ...
            # Load mask frame for video
            if os.path.isdir(path2):
                if not hasattr(self, 'mask_files') or self.frame == 1:
                    self.mask_files = sorted(glob.glob(os.path.join(path2, '*.*')))
                if self.frame <= len(self.mask_files):
                    img02 = cv2.imread(self.mask_files[self.frame - 1])
                else:
                    img02 = np.zeros_like(img0)
            elif os.path.isfile(path2) and any(path2.lower().endswith(x) for x in VID_FORMATS):
                if not hasattr(self, 'cap2') or self.frame == 1:
                    if hasattr(self, 'cap2'): self.cap2.release()
                    self.cap2 = cv2.VideoCapture(path2)
                ret_val2, img02 = self.cap2.read()
                if not ret_val2:
                    img02 = np.zeros_like(img0)
            else:
                img02 = np.zeros_like(img0)
...
def img2label_paths2(img_paths):
    ...
    # Check if 'images2' exists in paths, if not, try 'ARD100_mask32'
    if any(sa in x for x in img_paths):
        return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
    sa = os.sep + 'ARD100_mask32' + os.sep
    return [sb.join(x.rsplit(sa, 1)).rsplit('.', 1)[0] + '.txt' for x in img_paths]
