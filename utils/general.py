...
def init_seeds(seed=0): #初始化随机数种子
...
def resample_segments(segments, n=1000):
    ...
    for i, s in enumerate(segments):
        ...
        xp = np.arange(len(s))
        segments[i] = np.concatenate([np.int_erp(x, xp, s[:, i]) for i in range(2)]).reshape(2, -1).T  # segment xy
    return segments
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # Settings
    min_wh, max_wh = 2, 7680  # (pixels) minimum and maximum box width and height 最小和最大预测的宽高
    max_nms = 30000  # maximum number of boxes into torchvision.ops.nms() 最多检测的物体
    time_limit = 10.0  # seconds to quit after后处理最长时间
    redundant = True  # require redundant detections
    multi_label &= nc > 1  # multiple labels per box (adds 0.5ms/img) #每个box有多个label
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf 计算最终的置信度
        x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf

        # Box (center x, center y, width, height) to (x1, y1, x2, y2) 改变框的表达形式
        box = xywh2xyxy(x[:, :4])
