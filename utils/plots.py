...
def feature_visualization2(features, model_type, model_id, feature_num=64,row=8,col=8):
    """
...
class Annotator:
    ...
    def box_label(self, box, label='', color=(0, 255, 255), txt_color=(255, 255, 255)):
...
def dual_image(images,targets, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    ...
    for i in range(i + 1):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], None, (0, 255, 255), width=2)  # borders
        if paths:
...
def plot_images(images,images2,targets, paths=None,paths2=None, fname='images.jpg',fname2='fir_images.jpg', names=None, max_size=1920, max_subplots=16):
    dual_image(images,targets, paths, fname, names=None, max_size=1920, max_subplots=16)
    dual_image(images2,targets, paths2, fname2, names=None, max_size=1920, max_subplots=16)


def plot_images2(images, targets, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    # Plot image grid with labels
    if isinstance(images, torch.Tensor):
        images = images.cpu().float().numpy()
    if isinstance(targets, torch.Tensor):
